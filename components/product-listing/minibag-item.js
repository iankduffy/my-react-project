...
const MiniBagItem = (props) => {
  ...
  let updateQty = (qty) => {
    ...
    const newProductList = [...state.cart.products]

    setState({cart: { products: newProductList }, total: updatePrice(newProductList, state)})
    Cookies.set('cart', {cart: { products: newProductList}, total: updatePrice(newProductList, state)})
  }
  ...
  <div className="o-dropdown--item container__row">
    ...
    <div className="container__column col-8 u-pad-h-sm o-dropdown--item__details">
      <p className="u-t-b">{product.productName.toUpperCase()}</p>
      <p>£{product.price}</p>
